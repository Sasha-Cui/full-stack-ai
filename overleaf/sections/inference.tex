...
\begin{itemize}
  \item \textbf{Provider diversity:} Access to major proprietary and open-weight providers through one interface.
...
\paragraph{Cost Structure.} Providers usually charge separately for input and output tokens, but the cheapest per-token option is not always the cheapest system overall. In practice, compare:
\begin{itemize}
  \item \textbf{Frontier models:} strongest quality, but highest latency and per-token cost.
  \item \textbf{Mid-tier models:} strong quality for most product and research tasks.
  \item \textbf{Small or distilled models:} lowest latency for routing, extraction, and classification.
  \item \textbf{Self-hosted open-weight models:} attractive at high volume, but they introduce infrastructure and operations overhead.
\end{itemize}
Because pricing changes quickly, always verify the provider's current documentation before budgeting.
...
\begin{itemize}
  ...
  \item \textbf{Text+image-to-text:} Vision-language models for image understanding
  \item \textbf{Text+image-to-text+image:} Multimodal systems that can also generate visual outputs
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Reasoning effort control:} Some APIs expose effort or deliberation controls for harder problems
\end{itemize}
